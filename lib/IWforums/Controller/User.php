...
class IWforums_Controller_User extends Zikula_Controller {
    ...
    public function access($args) {
        ...
        if (strpos($item['mod'], '$' . $uid . '$') !== false) return 4;

        // if user is not registered check if can access the forum only in readtable mode
        if ($uid == '-1' && strpos($item['grup'], '$-1|') !== false) return 1;
    ...
    public function crear_tema($args) {
        ...
        if (!SecurityUtil::confirmAuthKey('IWforums')) {
            return LogUtil::registerAuthidError(ModUtil::url('IWforums', 'user', 'forum',
                                                              array('fid' => $fid)));
        }
        // get forum information
        $registre = ModUtil::apiFunc('IWforums', 'user', 'get',
        ...
        if ($lidTema != false) {
            ...
            if ($registre['msgDelTime'] > 0 && !$moderator) $missatge .= '<br />' . $this->__('During the next ') . $registre['msgDelTime'] . $this->__(' minutes you can delete the topic.');
...
//        $view->assign('uname', $usersInfo[$u]);
    ...
    public function update_msg($args) {
        ...
        } else $nom_fitxer = $missatge['adjunt'];
    ...
    public function pager($args) {
        ...
        if (!isset($inici) || empty($inici)) $inici = 1;

        if (!isset($rpp) || empty($rpp)) $rpp = 10;
